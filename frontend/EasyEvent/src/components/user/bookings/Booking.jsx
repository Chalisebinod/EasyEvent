...
const Booking = () => {
  ...
  const [additionalServices, setAdditionalServices] = useState([{ name: "", description: "" }]);
  ...
  useEffect(() => {
    if (guestCount && userOfferedValue) {
      ...
      const offeredPerPlate = offerMode === "perPlate" ? offerVal : (count ? offerVal / count : 0);
  ...
  const handleGuestCountChange = (e) => {
    ...
    if (selectedHall && parseInt(value) > selectedHall.capacity) {
      setCapacityError(`Maximum capacity for ${selectedHall.name} is ${selectedHall.capacity} guests.`);
    } else {
      setCapacityError("");
    }
    setGuestCount(value);
  ...
  const handleUserOfferedValueChange = (e) => {
    ...
    const minOffer = offerMode === "perPlate" ? originalPrice * 0.7 : officialTotal * 0.7;
    if (numValue < minOffer) {
      setOfferError(`Minimum allowed offer is ${minOffer.toFixed(2)} (${offerMode === "perPlate" ? "per plate" : "total"}).`);
    } else {
      setOfferError("");
    }
  ...
  const addService = () => setAdditionalServices((prev) => [...prev, { name: "", description: "" }]);
  ...
  const handleSubmit = (e) => {
    ...
    const offeredPerPlate = offerMode === "perPlate"
    ...
    fetch("http://localhost:8000/api/booking/create", {
      ...
      .then((res) => {
        if (!res.ok) {
          return res.json().then((data) => { throw new Error(data.message || "Failed to create booking"); });
        }
        return res.json();
      })
      .then((data) => {
  ...
  const extraFoods = selectedHall && selectedHall.includedFood
    ? foods.filter((food) => !selectedHall.includedFood.some((inc) => inc._id === food._id))
    : foods;
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 bg-gradient-to-br from-blue-50 to-gray-100">
        ...
        <div className="max-w-screen-xl mx-auto bg-white shadow-lg rounded-xl p-10 my-8">
          <h1 className="text-4xl font-extrabold text-center text-gray-900 mb-12">Book Your Event</h1>
          <form onSubmit={handleSubmit} className="space-y-12">
            {/* Hall Selection */}
            <section>
              <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Select a Hall</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {halls.map((hall) => (
                  <div
                    key={hall._id}
                    className={`border rounded-xl p-4 cursor-pointer transition transform hover:scale-105 ${
                      selectedHall && selectedHall._id === hall._id ? "border-orange-500 shadow-2xl" : "border-gray-300"
                    } ${!hall.isAvailable ? "opacity-50 cursor-not-allowed" : ""}`}
                    onClick={() => {
                      if (!hall.isAvailable) return;
                      setSelectedHall(hall);
                      setOriginalPrice(hall.pricePerPlate || hall.basePricePerPlate);
                      setSelectedFoods([]);
                    }}
                  >
                    {hall.images && hall.images.length > 0 ? (
                      <img
                        src={`http://localhost:8000/${hall.images[0].replace(/\\/g, "/")}`}
                        alt={hall.name}
                    ...
                    <p className="text-center font-medium text-lg">{hall.name}</p>
                    <p className="text-center text-sm text-gray-600">Capacity: {hall.capacity}</p>
                    {!hall.isAvailable && <p className="text-center text-red-500 mt-2">Not Available</p>}
                  </div>
                ))}
              </div>
            </section>
            ...
            {/* Included Food Items */}
            {selectedHall && selectedHall.includedFood && selectedHall.includedFood.length > 0 && (
              <section>
                <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Included Food Items</h2>
                <div className="flex flex-wrap gap-4">
                  {selectedHall.includedFood.map((food) => (
                    <div key={food._id} className="px-4 py-2 bg-green-100 text-green-800 rounded-full">
                      {food.name}
                    </div>
                  ))}
                </div>
            ...
            {/* Event Details */}
            <section>
              <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Event Details</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  <label className="block text-gray-700 font-medium">Guest Count</label>
                  <input
                    type="number"
                    value={guestCount}
            ...
            <section>
              <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Additional Food Items</h2>
                ...
                {["All", "Veg", "Non Veg"].map((type) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full transition ${
                      selectedFoodType === type ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-800"
                    }`}
                  >
                    {type}
                  </button>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                {filteredFoods.map((food) => (
                  ...
                  >
                    <p className="text-center text-sm font-medium">{food.name}</p>
                    <p className="text-center text-xs text-gray-600">${food.price}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            {/* Additional Services */}
            <section>
              <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Additional Services</h2>
            ...
            {/* Pricing Section */}
            <section>
              <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2 mb-6">Pricing</h2>
              <div className="grid grid-cols-1 gap-8">
                {/* Official Pricing */}
                <div className="p-6 border rounded-xl bg-gray-50 shadow-inner">
                  <h3 className="text-xl font-semibold mb-4">Official Pricing</h3>
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      <label className="block text-gray-700 font-medium">Price per Plate</label>
                      <input
                        type="number"
                        value={originalPrice}
                        readOnly
                    ...
                    <div>
                      <label className="block text-gray-700 font-medium">Extra Food Cost</label>
                      <input
                        type="number"
                  ...
                  <div className="mt-4">
                    <label className="block text-gray-700 font-medium">Grand Official Price</label>
                    <input
                      type="number"
                      value={guestCount ? (parseFloat(originalPrice) * parseInt(guestCount) + extraFoodCost).toFixed(2) : 0}
                      readOnly
                      className="mt-2 w-full border border-gray-300 rounded-md p-2 bg-gray-100"
                    />
                  </div>
                ...
                <div className="p-6 border rounded-xl bg-gray-50 shadow-inner">
                  ...
                  <div className="mb-6">
                    <label className="block text-gray-700 font-medium mb-2">Select Offer Type</label>
                    <div className="flex space-x-4">
                      <button
                        ...
                        className={`px-4 py-2 rounded-full transition ${
                          offerMode === "perPlate" ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-800"
                        }`}
                      >
                        Per Plate
                      </button>
                      <button
                        ...
                        className={`px-4 py-2 rounded-full transition ${
                          offerMode === "total" ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-800"
                        }`}
                      >
                        Total
                      </button>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    {offerMode === "perPlate" ? (
                      <>
                        <div>
                          <label className="block text-gray-700 font-medium">Offered Price per Plate</label>
                          <input
                            type="number"
                        ...
                        <div>
                          <label className="block text-gray-700 font-medium">Total Offer</label>
                          <input
                            type="number"
                            value={guestCount && userOfferedValue ? guestCount * userOfferedValue : ""}
                            readOnly
                            className="mt-2 w-full border border-gray-300 rounded-md p-2 bg-gray-100"
                          />
                        </div>
                      ...
                      <>
                        <div>
                          <label className="block text-gray-700 font-medium">Offered Total Price</label>
                          <input
                            type="number"
                            value={userOfferedValue}
                            onChange={handleUserOfferedValueChange}
                        ...
                        <div>
                          <label className="block text-gray-700 font-medium">Equivalent Price per Plate</label>
                          <input
                            type="number"
                            value={guestCount && userOfferedValue ? (userOfferedValue / guestCount).toFixed(2) : ""}
                            readOnly
                            className="mt-2 w-full border border-gray-300 rounded-md p-2 bg-gray-100"
                          />
                        </div>
                  ...
                  {offerError && <p className="text-red-500 text-sm mt-2">{offerError}</p>}
                  <div className="mt-6 p-4 border-t">
                    <p className="text-gray-700">
                      Final Accepted Price per Plate: <span className="font-semibold">{finalPrice}</span>
                    </p>
                    <p className="text-gray-700">
                      Total Final Cost: <span className="font-semibold">{totalCostFinal}</span>
                    </p>
                    <p className="text-gray-700">
                      Extra Food Cost: <span className="font-semibold">${extraFoodCost.toFixed(2)}</span>{" "}
                      <span className="text-sm">(for your selected extra food)</span>
                    </p>
                    <p className="text-gray-700 text-xl">
                      Grand Total (Your Offer + Extras): <span className="font-bold">${grandTotal.toFixed(2)}</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="text-center">
              <button type="submit" className="px-10 py-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-full shadow-lg hover:shadow-2xl transition">
        ...
        {chatOpen && <ChatWidget partnerId={partnerIdFromState} onClose={closeChat} />}
      </div>
      <BottomNavbar />
    </div>
  );
