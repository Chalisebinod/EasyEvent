  ...
  return (
    <>
            ...
            <Box mt={2}>
              ...
              >
                {hall.features.map((feat, idx) => (
                  <FeatureChip key={idx} label={feat} size="small" icon={<Stars fontSize="small" />} />
                ))}
              </Paper>
            </Box>
          )}
            ...
            <Box mt={2}>
              ...
              >
                {hall.includedFood.map((food, idx) => (
                  <FeatureChip key={idx} label={food.name} size="small" icon={<Restaurant fontSize="small" />} />
                ))}
              </Paper>
            </Box>
          )}
          ...
          <Grid container alignItems="center" justifyContent="space-between">
            ...
            <Grid item>
              <HallStatusSwitch>
                ...
                <Switch size="small" checked={hall.isAvailable} onChange={handleToggleClick} color="primary" />
              </HallStatusSwitch>
            </Grid>
          </Grid>
        </CardFooter>
      ...
      <Dialog open={toggleDialogOpen} onClose={() => setToggleDialogOpen(false)}>
        <DialogTitle>{pendingAvailability ? "Activate Hall" : "Deactivate Hall"}</DialogTitle>
        <DialogContent>
          <Typography>
            Are you sure you want to change the status of "{hall.name}" to {pendingAvailability ? "Available" : "Unavailable"}?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setToggleDialogOpen(false)} variant="outlined" color="inherit">
            Cancel
          </Button>
          <Button onClick={handleConfirmToggle} variant="contained" color={pendingAvailability ? "success" : "error"}>
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
...
const Hall = () => {
  ...
  const handleEditHall = async (hallId, updatedData) => {
    try {
      ...
      const response = await axios.patch(`http://localhost:8000/api/halls/${hallId}`, updatedData, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  const handleToggleClick = (event, hall) => {
    event.stopPropagation();
    setToggleHall(hall);
    setPendingAvailability(event.target.checked);
    setToggleDialogOpen(true);
  ...
  const handleConfirmToggle = async () => {
    if (!toggleHall) return;
    try {
      await handleEditHall(toggleHall._id, { availability: pendingAvailability });
      toast.success("Hall status updated successfully!");
    } catch (err) {
      toast.error("Failed to update hall status.");
    } finally {
  ...
  const handleAddCustomFeature = () => {
    ...
    setNewHall({ ...newHall, features: [...newHall.features, customFeature.trim()] });
    setCustomFeature("");
  };
  ...
  const handleImageUpload = (e) => {
    ...
    setNewHall({ ...newHall, images: [...newHall.images, ...Array.from(e.target.files)] });
  };

  const removeImage = (index) => {
    setNewHall({ ...newHall, images: newHall.images.filter((_, i) => i !== index) });
  };
    ...
    <Box display="flex" minHeight="100vh" bgcolor="#f8f9fa">
      ...
      <Box flexGrow={1} p={2}>
            ...
            <Grid container spacing={2} mt={1}>
              <Grid item xs={12} md={6}>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={newHall.isAvailable}
                      onChange={(e) => setNewHall({ ...newHall, isAvailable: e.target.checked })}
                      color="primary"
                    />
                  }
                  label="Available"
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={6} mt={3}>
                ...
                <TextField
                  ...
                  fullWidth
                  variant="outlined"
                  value={(editHall.includedFood && editHall.includedFood.join(", ")) || ""}
                  onChange={(e) =>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={editHall.isAvailable}
                      onChange={(e) =>
                        setEditHall({ ...editHall, isAvailable: e.target.checked })
                      }
                      color="primary"
                    />
                  }
        ...
        <Dialog open={toggleDialogOpen} onClose={() => setToggleDialogOpen(false)}>
          ...
          <DialogContent>
            <Typography>
              Are you sure you want to change the status of "{toggleHall?.name}" to {pendingAvailability ? "Available" : "Unavailable"}?
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button
